...
---@field send_prompt string|false|nil     Keymap for sending text to the LLM.
...
---@tag sllm.send_prompt()
--- Send a prompt to the LLM
---
--- Prompt the LLM with a preexisting prompt.
--- If in visual mode, automatically adds
--- the selection to context before prompting.
---
---@param prompt_text string The text prompt to send
---@return nil

function Sllm.send_prompt(prompt_text)
    local user_input = prompt_text
    if not user_input or user_input == '' then
        H.notify('[sllm] empty prompt.', vim.log.levels.INFO)
        return
    end
        ...
        H.pick(display_list, { prompt = 'Select conversation to continue:' }, function(_, idx)
            ...
            for _, entry in ipairs(selected.entries) do
                ...
                if formatted and type(formatted) == 'table' and #formatted > 0 then
                    H.ui_append_to_llm_buffer(formatted)
                end
            end
